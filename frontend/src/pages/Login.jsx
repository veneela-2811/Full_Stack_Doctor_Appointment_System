...
const Login = () => {
  ...
  //variables to store the values of the input field
  ...
  //if users are logged in,we'll direct them to home page
  useEffect(()=>{
    if(token){
      navigate('/')
  ...
  return (
    <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center' >
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg'>
        ...
        {/* Show name box only when users sign up.For sign in/log in,mil and password are enough */}
        {
        ...
        {
          state==="Sign Up"?<p>Already have an account?<span onClick={()=>setState('Login')} className='text-[#5f6FFF] underline cursor-pointer'>LogIn here</span></p> : <p><span className='text-[#5f6FFF] underline cursor-pointer'>Click here</span>to create a new account </p>
        }
      </div>
    </form>
  )
